...
<template>
    <NavBar></NavBar>
    <h1 class="font-medium text-2xl m-2 mt-20 ">{{ name }} episodios</h1>
    <div v-for="(season, index) in seasons" :key="index" class="mb-28">
        ...
        <div v-for="episode in season">
            <Link :href="`/show/episode/${episode.id}`"
                class=" m-2 flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 ">

            <img class="object-cover w-full rounded-t-lg h-52  md:w-44  md:rounded-none md:rounded-s-lg"
                :src="episode.image ? episode.image.medium : '/noimage.png'" alt="">
            <div class="flex flex-col justify-between p-4 leading-normal">
                ...
                <p class="mb-3 font-light text-gray-700"
                    v-html="episode.summary.length > 200 ? episode.summary.slice(0, 200) + '...' : episode.summary"></p>

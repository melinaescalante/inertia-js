...
onMounted(async () => {
    unSubscribeFromAuth = suscribeToAuthChanged(
        (newUserData) => (loginUser.value = newUserData)
    );
});
defineProps({
    series: Array,
...
async function handlePost() {
    ...
    let imageURL;
    imageInput !== null
        ? await uploadPhoto(newPost.value.image)
        : (imageURL = null);
...
<template>
    ...
    <form action="" enctype="multipart/form-data" @submit.prevent="handlePost">
        <div class="flex flex-col mb-3">
            <span class="sr-only"
                >Escribe la serie con la que te quieres referir</span
            >
            <label class="m-2" for="serie"
                >¿Sobre qué serie estas pensando?</label
            >
            <!-- <SearchComponent :series="series">      </SearchComponent> -->
            <SearchComponentPost
                @serie-seleccionada="setSerieSeleccionada"
                :series="series"
        ...
        <div class="flex flex-col mb-3">
            ...
            <textarea
                :readonly="loading"
                class="p-2 m-2 border rounded-md"
                name="description"
                id="description"

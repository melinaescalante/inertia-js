...
<template>
    ...
    <div v-if="msg !== 'Se ha publicado correctamente' && msg !== ''"
        class="bg-red-200 p-4 m-2 rounded-md skiptranslate">
        {{ msg }}
    </div>

    <div id="boolean-success-msg-uploadpost" v-show="msgBoolean" v-if="msg == 'Se ha publicado correctamente'"
        class="bg-green-200 fixed w-[-webkit-fill-available] flex items-center top-[10%]  p-4 m-2 rounded-md skiptranslate">
        <svg class="  w-6 h-6   me-5" aria-hidden="true" stroke-width="2" xmlns="http://www.w3.org/2000/svg" fill="none"
            stroke=" #1f2937 " viewBox="0 0 22 20">
            <path

...
<template>
    ...
    <div class="flex gap-3 m-2">
        <img class="" :src="serie.image.medium ? serie.image.medium : 'noimage.png'"
        ...
        <div>
            ...
            <p>Estreno: {{ serie.premiered.lenght > 4 ? `${serie.premiered.slice(0, 5)}` : serie.premiered }}</p>
        </div>
    </div>
    <div class="flex m-2 mt-3 justify-between  ">
        <ButtonBase>Episodios</ButtonBase>
        <ButtonBase
        :href="serie.id"
        :name="serie.name"
        param="temporadas"
        >Temporadas</ButtonBase>
        <ButtonBase>Galeria</ButtonBase>
        <ButtonBase
        :href="serie.id"
        :name="serie.name"
        param="elenco"
